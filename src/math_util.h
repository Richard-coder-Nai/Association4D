...
namespace MathUtil
{
	...
	float Point2PointDist(const Eigen::Vector3f& pA, const Eigen::Vector3f& pB)
	{
		return (pA(0) - pB(0))*(pA(0) - pB(0)) + (pA(1) - pB(1))*(pA(1) - pB(1));
	}

	float Point2PointDist3D(const Eigen::Vector3f& pA, const Eigen::Vector3f& pB)
	{
	...
	{
		return ((pA - pB).cross(ray)).norm();//取模，没有约化过的距离
	}
