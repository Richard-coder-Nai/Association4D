...
std::vector<Person3D> maintainPerson3D(const std::vector<Person3D>& lastPersons3D)
{
	std::vector<Person3D> lastPersons3D_out = lastPersons3D;
	for (auto person = lastPersons3D_out.begin(); person != lastPersons3D_out.end();)
	{
		//std::cout << "3D person joint count is"<<person->GetJointCnt() << std::endl;
		
		if (person->GetJointCnt() < 0.8*GetSkelDef().jointSize)
		{
			lastPersons3D_out.erase(person);
		}
		else person++;
	}
	return lastPersons3D_out;
}
...
int main()
{
	...
	std::vector<Person3D> lastPerson3D;
	
	for (int frameIdx = 0; frameIdx < 300; frameIdx++) {
		for (int camIdx = 0; camIdx < cameras.size(); camIdx++)
			videos[camIdx] >> rawImgs[camIdx];//把videos按流的方式输入rawImgs
		if (frameIdx == 0)
		{
			
			associater.SetDetection(detections[frameIdx]);
			...
			associater.ProposalCollocation();//生成2D人连成3D人的可能
		...
		else
		{
			...
			associater.ConstructJointRays();//根据关键点在当前机位的投影构造三维投影线
			associater.ConstructJointEpiEdges();//计算相同种类之间连线的距离
			associater.MatchPersons4D(lastPerson3D);
			associater.NaiveTimeTrackingPerson3D(lastPerson3D);
			associater.FinishPersonsMap();
			associater.ClusterPersons2D();
			associater.ProposalCollocation();//生成2D人连成3D人的可能
			associater.ClusterPersons3D();
			associater.ConstructPersons();
		...
		lastPerson3D = maintainPerson3D(associater.GetPersons3D());
	}
