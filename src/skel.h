...
struct Person2D
{
	...
	float CalcScore() const { return 1.2*joints.row(2).sum() + pafs.sum(); }
...
struct Person3D
{
	Eigen::Matrix4Xf joints;//(x,y,z,1)valid;(...,0)invalid
	Person3D() {
	...
	int GetJointCnt() const { return int((joints.row(3).array() > 0.f).count()); }
};
...
struct SkelDetection
{
	...
	std::vector<Eigen::Matrix3Xf> joints;//3 rows
	std::vector<Eigen::MatrixXf> pafs;//unknown size
};
